...
def test_add_guest_waits_when_no_table_fits(client, occupy_all_tables):
    ...
    response = client.post('/api/waitlist', json={'name': 'C', 'party_size': 2, 'phone': '3', 'notes': ''})
    guest = response.json()
...
def test_larger_party_is_prioritized_over_earlier_smaller_party(client, occupy_all_tables):
    ...
    small = client.post(
        '/api/waitlist', json={'name': 'Small', 'party_size': 2, 'phone': '1', 'notes': ''}
    ).json()
    big = client.post(
        '/api/waitlist', json={'name': 'Big', 'party_size': 4, 'phone': '2', 'notes': ''}
    ).json()
    ...
    client.post('/api/tables/t3/confirm-free')

    listed = {g['id']: g for g in client.get('/api/waitlist').json()}
...
def test_list_waitlist_returns_guests_in_creation_order(client):
    client.post('/api/waitlist', json={'name': 'First', 'party_size': 2, 'phone': '1', 'notes': ''})
    client.post('/api/waitlist', json={'name': 'Second', 'party_size': 2, 'phone': '2', 'notes': ''})

    names = [g['name'] for g in client.get('/api/waitlist').json()]
...
def test_seat_guest_transitions_status(client):
    ...
    response = client.post(f"/api/waitlist/{guest['id']}/seat")
...
def test_seat_guest_not_ready_yet_is_conflict(client, occupy_all_tables):
    ...
    waiting_guest = client.post(
        '/api/waitlist', json={'name': 'C', 'party_size': 2, 'phone': '3', 'notes': ''}
    ).json()
    assert waiting_guest['status'] == 'waiting'

    response = client.post(f"/api/waitlist/{waiting_guest['id']}/seat")

    assert response.status_code == 409


def test_seat_unknown_guest_is_404(client):
    response = client.post('/api/waitlist/does-not-exist/seat')
...
def test_cancel_guest_frees_up_the_table_for_the_next_waiting_guest(client, occupy_all_tables):
    ...
    first = client.post(
        '/api/waitlist', json={'name': 'First', 'party_size': 2, 'phone': '1', 'notes': ''}
    ).json()
    assert first['status'] == 'table_ready'
    assert first['assigned_table_id'] == 't1'

    second = client.post(
        '/api/waitlist', json={'name': 'Second', 'party_size': 2, 'phone': '2', 'notes': ''}
    ).json()
    third = client.post(
        '/api/waitlist', json={'name': 'Third', 'party_size': 2, 'phone': '3', 'notes': ''}
    ).json()
    assert second['status'] == 'waiting'
    assert third['status'] == 'waiting'

    response = client.post(f"/api/waitlist/{first['id']}/cancel")
    assert response.status_code == 200
...
def test_cancel_unknown_guest_is_404(client):
    response = client.post('/api/waitlist/does-not-exist/cancel')
